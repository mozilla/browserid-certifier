...
// if th variable is not set, try the default installation path, and if 
// it does not exist, try a relative path
if (process.env['CONFIG_FILES']) {
...
} else if (path.existsSync('/opt/certifier/config/local.json')) {
    var c = cjson.load('/opt/certifier/config/local.json');
    conf.load(c);
} else {
    var c = cjson.load('config/local.json');
    conf.load(c);
}
